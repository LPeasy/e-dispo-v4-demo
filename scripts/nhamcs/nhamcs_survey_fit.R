...
# Survey-weighted NHAMCS logistic fitting entry point.
#
...
if (length(args) < 2) {
  stop("Usage: nhamcs_survey_fit.R <analytic_cohort_csv> <output_dir>")
}
...
MODEL_SPECS <- list(
  list(
    ...
    terms = list(
      ...
      list(kind = "binary", field = "vomiting", reference = "0"),
  ...
  list(
    ...
    terms = list(
      list(kind = "categorical", field = "age_band", reference = "30_44"),
      list(kind = "continuous", field = "pain_score", reference = "mean_centered_scaled"),
      list(kind = "binary", field = "pain_missing", reference = "0"),
      list(kind = "binary", field = "vomiting", reference = "0"),
      list(kind = "binary", field = "fever_or_temp", reference = "0")
    )
...
required <- c("include_strict_binary", "weight", "stratum", "psu", "admit", "age_band", "pain_bin3", "pain_missing", "fever_or_temp")
...
for (field in intersect(c("pain_score", "pain_missing", "temp", "fever_or_temp", "vomiting", "acuity", "HR", "SBP"), names(binary))) {
...
write_metadata <- function(model_results) {
  metadata <- list(
    ...
    survey_design = list(
      weight = "PATWT mapped to weight",
      stratum = "CSTRATM mapped to stratum",
      psu = "CPSUM mapped to psu",
...
sparse_notes <- function(binary) {
  ...
  if ("vomiting" %in% names(binary) && !has_variation(as.numeric(binary$vomiting))) {
    notes <- c(notes, "Vomiting mapping unavailable or has no usable variation; vomiting term removed where needed.")
  }
