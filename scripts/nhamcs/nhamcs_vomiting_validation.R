...
# Vomiting-only validation gate for pooled NHAMCS.
# Ordinary vomiting is RFV 15300 only. Hematemesis is a separate future candidate.

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
  stop("Usage: nhamcs_vomiting_validation.R <pooled_analytic_cohort_csv> <output_dir>")
}
...
required <- c("include_strict_binary", "admit", "year", "age", "pain_bin3", "pain_missing",
              "fever_or_temp", "vomiting_present", "vomiting_rfv_position_tier",
...
binary$vomiting_rfv_position_tier[is.na(binary$vomiting_rfv_position_tier) | binary$vomiting_rfv_position_tier == ""] <- "none"
binary$vomiting_rfv_position_tier <- factor(binary$vomiting_rfv_position_tier, levels = c("none", "secondary_rfv2_5", "primary_rfv1"))
...
MODEL_SPECS <- list(
  list(model_id = "vomiting_base_complete_temp", formula = admit ~ age_centered + pain_bin3 + pain_missing + fever_or_temp),
  list(model_id = "vomiting_candidate_binary_complete_temp", formula = admit ~ age_centered + pain_bin3 + pain_missing + fever_or_temp + vomiting_present),
  list(model_id = "vomiting_tier_sensitivity_complete_temp", formula = admit ~ age_centered + pain_bin3 + pain_missing + fever_or_temp + vomiting_rfv_position_tier)
)
...
parse_coefficient <- function(name) {
  ...
  if (startsWith(name, "vomiting_rfv_position_tier")) {
    return(list(term = "vomiting_rfv_position_tier", level = substring(name, nchar("vomiting_rfv_position_tier") + 1)))
  }
...
fit_one_model <- function(spec, frame) {
  ...
  if (nrow(model_frame) == 0 || length(unique(model_frame$admit)) < 2) {
    return(blocked("blocked_no_outcome_variation", "No usable binary outcome variation after vomiting-gate model filtering."))
  }
  ...
  coefficients <- data.frame(model_id = spec$model_id, dataset = DATASET,
    ...
    limitation_note = "Vomiting validation only; no coefficient promotion or app activation.",
  ...
  calibration <- data.frame(model_id = spec$model_id, dataset = DATASET,
    ...
    notes = "Survey-weighted apparent calibration for vomiting validation.", stringsAsFactors = FALSE)
...
counts_for_frame <- function(frame, year_value = "pooled") {
  rows <- data.frame()
  rows <- rbind(rows, count_row(frame, "vomiting_present", "0", frame$vomiting_present == 0, year_value))
  rows <- rbind(rows, count_row(frame, "vomiting_present", "1", frame$vomiting_present == 1, year_value))
  for (level in c("none", "secondary_rfv2_5", "primary_rfv1")) {
    rows <- rbind(rows, count_row(frame, "vomiting_rfv_position_tier", level, as.character(frame$vomiting_rfv_position_tier) == level, year_value))
  }
...
heldout_metric_row <- function(spec, train, test, heldout_year) {
  train_vomit <- train[train$vomiting_present == 1, , drop = FALSE]
  test_vomit <- test[test$vomiting_present == 1, , drop = FALSE]
  blocked_row <- function(status, note) data.frame(heldout_year = heldout_year, model_id = spec$model_id, dataset = DATASET,
    ...
    train_vomiting_positive_n = nrow(train_vomit),
    train_vomiting_positive_events = if (nrow(train_vomit) > 0) sum(train_vomit$admit == 1) else 0,
    train_vomiting_positive_non_events = if (nrow(train_vomit) > 0) sum(train_vomit$admit == 0) else 0,
    heldout_n = nrow(test), heldout_events = if (nrow(test) > 0) sum(test$admit) else 0,
    heldout_vomiting_positive_n = nrow(test_vomit),
    heldout_vomiting_positive_events = if (nrow(test_vomit) > 0) sum(test_vomit$admit == 1) else 0,
    heldout_vomiting_positive_non_events = if (nrow(test_vomit) > 0) sum(test_vomit$admit == 0) else 0,
    observed_prevalence = NA_real_, mean_predicted = NA_real_, calibration_in_the_large = NA_real_,
    calibration_slope = NA_real_, brier_score = NA_real_, auroc = NA_real_, vomiting_beta = NA_real_,
    vomiting_se = NA_real_, vomiting_or = NA_real_, vomiting_ci_low = NA_real_, vomiting_ci_high = NA_real_,
  ...
  if (nrow(train) == 0 || nrow(test) == 0 || length(unique(train$admit)) < 2) {
    return(blocked_row("blocked_no_train_or_test_outcome_variation", "Leave-one-year-out vomiting validation."))
  }
  result <- fit_one_model(spec, train)
  if (is.null(result$fit)) return(blocked_row("blocked_fit_failed", paste("Leave-one-year-out vomiting validation fit failed:", result$fit_error)))
  ...
  vomit <- coefficient_row(result$coefficients, spec$model_id, "vomiting_present", "1")
  data.frame(heldout_year = heldout_year, model_id = spec$model_id, dataset = DATASET,
    train_n = nrow(train), train_events = sum(train$admit), train_vomiting_positive_n = nrow(train_vomit),
    train_vomiting_positive_events = if (nrow(train_vomit) > 0) sum(train_vomit$admit == 1) else 0,
    train_vomiting_positive_non_events = if (nrow(train_vomit) > 0) sum(train_vomit$admit == 0) else 0,
    heldout_n = nrow(test), heldout_events = sum(test$admit), heldout_vomiting_positive_n = nrow(test_vomit),
    heldout_vomiting_positive_events = if (nrow(test_vomit) > 0) sum(test_vomit$admit == 1) else 0,
    heldout_vomiting_positive_non_events = if (nrow(test_vomit) > 0) sum(test_vomit$admit == 0) else 0,
    observed_prevalence = weighted_mean(y, weight), mean_predicted = weighted_mean(predicted, weight),
    calibration_in_the_large = calibration_intercept, calibration_slope = calibration_slope,
    brier_score = weighted_mean((predicted - y) ^ 2, weight), auroc = weighted_auc(y, predicted, weight),
    vomiting_beta = if (is.null(vomit)) NA_real_ else vomit$beta[[1]],
    vomiting_se = if (is.null(vomit)) NA_real_ else vomit$se[[1]],
    vomiting_or = if (is.null(vomit)) NA_real_ else vomit$odds_ratio[[1]],
    vomiting_ci_low = if (is.null(vomit)) NA_real_ else vomit$ci_low[[1]],
    vomiting_ci_high = if (is.null(vomit)) NA_real_ else vomit$ci_high[[1]],
    ...
    notes = "Leave-one-year-out vomiting validation on complete-temperature rows.", stringsAsFactors = FALSE)
}

leave_one_year_out <- data.frame()
validation_specs <- MODEL_SPECS[vapply(MODEL_SPECS, function(spec) spec$model_id %in% c("vomiting_base_complete_temp", "vomiting_candidate_binary_complete_temp"), logical(1))]
...
base_calibration <- calibration_row("vomiting_base_complete_temp")
candidate_calibration <- calibration_row("vomiting_candidate_binary_complete_temp")
tier_calibration <- calibration_row("vomiting_tier_sensitivity_complete_temp")
pooled_vomit_counts <- cell_counts[cell_counts$year == "pooled" & cell_counts$group == "vomiting_present" & cell_counts$level == "1", , drop = FALSE]
pooled_vomiting_positive_n <- if (nrow(pooled_vomit_counts) > 0) pooled_vomit_counts$n[[1]] else 0
pooled_vomiting_positive_events <- if (nrow(pooled_vomit_counts) > 0) pooled_vomit_counts$admissions[[1]] else 0
pooled_vomiting_positive_nonevents <- if (nrow(pooled_vomit_counts) > 0) pooled_vomit_counts$non_admissions[[1]] else 0
vomiting_coef <- coefficient_row(coefficients, "vomiting_candidate_binary_complete_temp", "vomiting_present", "1")
vomiting_draws <- draws[draws$model_id == "vomiting_candidate_binary_complete_temp" & draws$term == "vomiting_present", , drop = FALSE]
prob_beta_positive <- if (nrow(vomiting_draws) > 0) mean(vomiting_draws$beta > 0) else NA_real_

candidate_loo <- leave_one_year_out[leave_one_year_out$model_id == "vomiting_candidate_binary_complete_temp", , drop = FALSE]
base_loo <- leave_one_year_out[leave_one_year_out$model_id == "vomiting_base_complete_temp", , drop = FALSE]
merged_loo <- merge(base_loo, candidate_loo, by = "heldout_year", suffixes = c("_base", "_vomiting"), sort = TRUE)
heldout_brier_worsening <- if (nrow(merged_loo) > 0) weighted_mean(merged_loo$brier_score_vomiting - merged_loo$brier_score_base, merged_loo$heldout_n_base) else NA_real_
heldout_auroc_change <- if (nrow(merged_loo) > 0) weighted_mean(merged_loo$auroc_vomiting - merged_loo$auroc_base, merged_loo$heldout_n_base) else NA_real_
heldout_status_degraded <- if (nrow(merged_loo) > 0) any(mapply(status_degraded, merged_loo$pass_fail_status_base, merged_loo$pass_fail_status_vomiting), na.rm = TRUE) else TRUE
...
positive_coefficient_gate <- !is.null(vomiting_coef) && is.finite(vomiting_coef$beta[[1]]) && vomiting_coef$beta[[1]] > 0
uncertainty_exported_gate <- !is.null(vomiting_coef) && nrow(covariance[covariance$model_id == "vomiting_candidate_binary_complete_temp", , drop = FALSE]) > 0 &&
  length(unique(vomiting_draws$draw_id)) == DRAW_COUNT && is.finite(prob_beta_positive) && prob_beta_positive >= MIN_PROB_BETA_POSITIVE
sparse_cell_gate <- pooled_vomiting_positive_n >= MIN_POSITIVE_N &&
  pooled_vomiting_positive_events >= MIN_POSITIVE_EVENTS &&
  pooled_vomiting_positive_nonevents >= MIN_POSITIVE_NONEVENTS
training_split_gate <- nrow(candidate_loo) == length(unique(complete_temp$year)) &&
  all(candidate_loo$train_vomiting_positive_events >= MIN_POSITIVE_EVENTS, na.rm = FALSE) &&
  all(candidate_loo$train_vomiting_positive_non_events >= MIN_POSITIVE_NONEVENTS, na.rm = FALSE)
loo_stability_gate <- nrow(candidate_loo) == length(unique(complete_temp$year)) &&
  ...
  all(is.finite(candidate_loo$vomiting_beta)) && all(candidate_loo$vomiting_beta > 0) &&
  all(is.finite(candidate_loo$vomiting_ci_high)) && all(candidate_loo$vomiting_ci_high <= MAX_OR_CI_HIGH)
...
documentation_gate <- all(complete_temp$vomiting_present %in% c(0, 1)) &&
  ...
  all(as.character(complete_temp$vomiting_rfv_position_tier) %in% c("none", "secondary_rfv2_5", "primary_rfv1"))

tier_counts <- cell_counts[cell_counts$year == "pooled" & cell_counts$group == "vomiting_rfv_position_tier" &
...
recommended_form <- if (tier_clearly_better) "tiered_rfv_position_sensitivity" else "binary_vomiting_present"

mapping_or_fitting_blocked <- is.null(vomiting_coef) || is.null(base_calibration) || is.null(candidate_calibration) || !documentation_gate
...
tier_sensitivity <- data.frame(
  dataset = DATASET,
  binary_model_id = "vomiting_candidate_binary_complete_temp",
  tier_model_id = "vomiting_tier_sensitivity_complete_temp",
  ...
  notes = "RFV position is documentation prominence, not vomiting severity.",
  evidence_tier = "survey_weighted_candidate",
  stringsAsFactors = FALSE
)
...
gate_rows <- data.frame(
  ...
  status = c(ifelse(positive_coefficient_gate, "pass", "fail"), ifelse(uncertainty_exported_gate, "pass", "fail"),
    ...
    promotion_status),
  metric = c(if (!is.null(vomiting_coef)) vomiting_coef$beta[[1]] else NA_real_, prob_beta_positive,
    pooled_vomiting_positive_events, if (nrow(candidate_loo) > 0) min(candidate_loo$train_vomiting_positive_events) else NA_real_,
    if (nrow(candidate_loo) > 0) max(candidate_loo$vomiting_ci_high, na.rm = TRUE) else NA_real_,
    apparent_brier_worsening, sum(complete_temp$vomiting_present == 1), tier_brier_worsening_vs_binary, NA_real_),
  threshold = c("> 0", paste0(">= ", MIN_PROB_BETA_POSITIVE, " with ", DRAW_COUNT, " draws"),
    paste0("vomiting n >= ", MIN_POSITIVE_N, "; events >= ", MIN_POSITIVE_EVENTS, "; non-events >= ", MIN_POSITIVE_NONEVENTS),
    paste0("each training split vomiting events/non-events >= ", MIN_POSITIVE_EVENTS, "/", MIN_POSITIVE_NONEVENTS),
    paste0("all LOO vomiting betas > 0 and OR CI high <= ", MAX_OR_CI_HIGH),
    ...
    "all binary vomiting gates pass"),
  details = c(
    if (!is.null(vomiting_coef)) paste0("beta=", signif(vomiting_coef$beta[[1]], 6), ", se=", signif(vomiting_coef$se[[1]], 6), ", OR=", signif(vomiting_coef$odds_ratio[[1]], 6)) else "vomiting coefficient unavailable",
    paste0("draws=", length(unique(vomiting_draws$draw_id)), ", Pr(beta>0)=", signif(prob_beta_positive, 6)),
    paste0("pooled vomiting n=", pooled_vomiting_positive_n, ", events=", pooled_vomiting_positive_events, ", non-events=", pooled_vomiting_positive_nonevents),
    if (nrow(candidate_loo) > 0) paste0("minimum train vomiting events=", min(candidate_loo$train_vomiting_positive_events), ", minimum train vomiting non-events=", min(candidate_loo$train_vomiting_positive_non_events)) else "LOO rows unavailable",
    if (nrow(candidate_loo) > 0) paste0("finite positive beta years=", sum(is.finite(candidate_loo$vomiting_beta) & candidate_loo$vomiting_beta > 0), "/", nrow(candidate_loo), ", max OR CI high=", signif(max(candidate_loo$vomiting_ci_high, na.rm = TRUE), 6)) else "LOO rows unavailable",
    ...
    paste0("vomiting positives=", sum(complete_temp$vomiting_present == 1), ", hematemesis positives kept separate=", sum(complete_temp$hematemesis_present == 1)),
    paste0("recommended form=", recommended_form, ", tier cell gate=", tier_cell_gate),
    paste0("final vomiting promotion gate status: ", promotion_status)),
  evidence_tier = "survey_weighted_candidate",
  stringsAsFactors = FALSE
)
...
report <- c(
  "# Vomiting RFV Validation Gate", "",
  paste0("Dataset: `", DATASET, "`"), "",
  paste0("Decision: `", promotion_status, "`."), "",
  "This gate evaluates ordinary vomiting from RFV code `15300` only. `15802` hematemesis is tabulated separately and is not included in the vomiting promotion model.",
  "", "## Gate Summary", "", markdown_gate_table(gate_rows),
  "", "## Model Comparison", "", markdown_calibration_table(calibration),
  "", "## Vomiting Coefficient", "",
  if (!is.null(vomiting_coef)) paste0("- Pooled vomiting coefficient: beta ", format_number(vomiting_coef$beta[[1]], 4),
    ", SE ", format_number(vomiting_coef$se[[1]], 4), ", OR ", format_number(vomiting_coef$odds_ratio[[1]], 3),
    ", 95% CI ", format_number(vomiting_coef$ci_low[[1]], 3), " to ", format_number(vomiting_coef$ci_high[[1]], 3), ".") else "- Pooled vomiting coefficient was not estimated.",
  paste0("- MVN draw probability beta > 0: ", format_number(prob_beta_positive, 4), "."),
  "", "## Sparse Cells", "",
  paste0("- Pooled vomiting-positive N: ", pooled_vomiting_positive_n, "."),
  paste0("- Pooled vomiting-positive admissions: ", pooled_vomiting_positive_events, "."),
  paste0("- Pooled vomiting-positive non-admissions: ", pooled_vomiting_positive_nonevents, "."),
  paste0("- Pooled hematemesis-positive N kept separate: ", sum(complete_temp$hematemesis_present == 1), "."),
  "", "## Tier Sensitivity", "",
  paste0("- Recommended form: `", recommended_form, "`."),
  "- RFV position is documentation prominence/order, not vomiting severity.",
  "", "## Boundary", "",
  "- Passing this gate means vomiting is eligible for later evidence-engine review, not automatically promoted.",
  "- Failing this gate keeps vomiting as sensitivity-only or blocked according to the decision row.",
  "- This output does not claim clinical validity."
)

utils::write.csv(cell_counts, file.path(output_dir, "vomiting_cell_counts_by_year.csv"), row.names = FALSE, na = "")
utils::write.csv(unadjusted_rates, file.path(output_dir, "vomiting_unadjusted_rates_by_year.csv"), row.names = FALSE, na = "")
utils::write.csv(coefficients, file.path(output_dir, "vomiting_model_comparison_coefficients.csv"), row.names = FALSE, na = "")
utils::write.csv(covariance, file.path(output_dir, "vomiting_model_comparison_covariance.csv"), row.names = FALSE, na = "")
utils::write.csv(draws, file.path(output_dir, "vomiting_model_comparison_draws.csv"), row.names = FALSE, na = "")
utils::write.csv(calibration, file.path(output_dir, "vomiting_model_comparison_calibration.csv"), row.names = FALSE, na = "")
utils::write.csv(deciles, file.path(output_dir, "vomiting_model_comparison_deciles.csv"), row.names = FALSE, na = "")
utils::write.csv(leave_one_year_out, file.path(output_dir, "vomiting_leave_one_year_out_validation.csv"), row.names = FALSE, na = "")
utils::write.csv(tier_sensitivity, file.path(output_dir, "vomiting_tier_sensitivity.csv"), row.names = FALSE, na = "")
utils::write.csv(gate_rows, file.path(output_dir, "vomiting_gate_decision.csv"), row.names = FALSE, na = "")
writeLines(report, con = file.path(output_dir, "vomiting_validation_report.md"))

cat("Wrote vomiting validation outputs under ", output_dir, "\n", sep = "")
