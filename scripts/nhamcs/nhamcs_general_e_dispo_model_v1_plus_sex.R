...
# General E-Dispo model v1 plus-sex sensitivity.
#
# This is a prespecified sensitivity model that adds sex as a main effect to
# the parallel all-sex/all-age non-trauma NHAMCS educational/statistical model.
# It is not the active e-dispo-v4.0 app model and it does not replace the
# adult-male abdominal-pain model or the base general-E-Dispo-model-v1 artifact.

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
  stop("Usage: nhamcs_general_e_dispo_model_v1_plus_sex.R <full_source_nontrauma_cohort_csv> <output_dir>")
}
...
OUTPUT_FILES <- c(
  coefficients = "general_e_dispo_model_v1_plus_sex_coefficients.csv",
  covariance = "general_e_dispo_model_v1_plus_sex_covariance.csv",
  performance = "general_e_dispo_model_v1_plus_sex_performance.csv",
  calibration = "general_e_dispo_model_v1_plus_sex_calibration.csv",
  calibration_by_decile = "general_e_dispo_model_v1_plus_sex_calibration_by_decile.csv",
  performance_intervals = "general_e_dispo_model_v1_plus_sex_performance_intervals.csv",
  optimism = "general_e_dispo_model_v1_plus_sex_optimism_corrected_performance.csv",
  subgroup_performance = "general_e_dispo_model_v1_plus_sex_subgroup_performance.csv",
  subgroup_calibration = "general_e_dispo_model_v1_plus_sex_subgroup_calibration.csv",
  missingness = "general_e_dispo_model_v1_plus_sex_missingness.csv",
  spec = "general_e_dispo_model_v1_plus_sex_model_spec.json",
  report = "general_e_dispo_model_v1_plus_sex_report.md",
  comparison = "general_e_dispo_model_v1_sex_sensitivity_comparison.csv"
)

FORMULA_TEXT <- paste(
  "admit ~ age_centered_40 + sex + acuity_code + arrival_transfer_context +",
...
write_interval_blocker <- function(blocker) {
  path <- file.path(output_dir, "general_e_dispo_model_v1_plus_sex_performance_intervals_blocker.md")
  if (!nzchar(blocker)) {
    if (file.exists(path)) unlink(path)
    return(invisible(NULL))
  }
  writeLines(c(
    "# general-E-Dispo-model-v1-plus-sex Performance Interval Blocker",
    "",
    ...
    paste0("- ", blocker),
    "",
    "No interval value should be inferred for blocked rows in `general_e_dispo_model_v1_plus_sex_performance_intervals.csv`.",
    ""
  ), con = path)
}
...
grouped_calibration <- function(frame, predicted) {
  ...
  for (group in sort(unique(frame$.__calibration_decile[!is.na(frame$.__calibration_decile)]))) {
    ...
    rows <- rbind(rows, data.frame(
      ...
      notes = "Apparent grouped calibration for general-E-Dispo-model-v1-plus-sex; no external validation or transportability claim.",
      stringsAsFactors = FALSE
    ))
  }
  rows
...
write_optimism_blocker <- function(reason) {
  path <- file.path(output_dir, "general_e_dispo_model_v1_plus_sex_optimism_corrected_performance_blocker.md")
  if (!nzchar(reason)) {
    if (file.exists(path)) unlink(path)
    return(invisible(NULL))
  }
  writeLines(c(
    "# general-E-Dispo-model-v1-plus-sex Optimism-Corrected Performance Blocker",
    "",
    ...
    paste0("- ", reason),
    "",
    "Use `general_e_dispo_model_v1_plus_sex_performance_intervals.csv` as the current fixed-prediction interval artifact.",
    ""
  ), con = path)
}
...
comparison_row <- function(model_id, role, performance, calibration, optimism, status, limitation) {
  data.frame(
    model_id = model_id,
...
delta_row <- function(base_row, plus_row) {
  numeric_columns <- c(
  ...
  row <- plus_row
  row$model_id <- "general-E-Dispo-model-v1-plus-sex_minus_base"
  row$comparison_role <- "plus_sex_minus_base"
  for (column_name in numeric_columns) {
    row[[column_name]] <- as.numeric(plus_row[[column_name]]) - as.numeric(base_row[[column_name]])
  }
  row$status <- ifelse(
  ...
  row$limitation <- "Metric deltas compare the plus-sex sensitivity model against the base general-E-Dispo-model-v1 apparent/internal NHAMCS artifacts only."
  row
}

write_sensitivity_comparison <- function(performance, calibration, optimism, limitation_note) {
  base_performance_path <- file.path(output_dir, "general_e_dispo_model_v1_performance.csv")
  base_calibration_path <- file.path(output_dir, "general_e_dispo_model_v1_calibration.csv")
  base_optimism_path <- file.path(output_dir, "general_e_dispo_model_v1_optimism_corrected_performance.csv")

  plus_row <- comparison_row(
    MODEL_ID,
    "plus_sex_sensitivity",
    performance,
    calibration,
    optimism,
    as.character(first_column_value(performance, "status", "unknown")),
    limitation_note
  )

  if (file.exists(base_performance_path) && file.exists(base_calibration_path) && file.exists(base_optimism_path)) {
    base_performance <- read.csv(base_performance_path, stringsAsFactors = FALSE)
    base_calibration <- read.csv(base_calibration_path, stringsAsFactors = FALSE)
    base_optimism <- read.csv(base_optimism_path, stringsAsFactors = FALSE)
    base_row <- comparison_row(
      "general-E-Dispo-model-v1",
      "base_general_model",
      base_performance,
      base_calibration,
      base_optimism,
      as.character(first_column_value(base_performance, "status", "unknown")),
      "Base general model artifact excludes sex from the fitted formula."
    )
    comparison <- rbind(base_row, plus_row, delta_row(base_row, plus_row))
  } else {
    base_row <- comparison_row(
      "general-E-Dispo-model-v1",
      "base_general_model",
      ...
      NULL,
      NULL,
      "blocked_base_artifacts_missing",
      "Base general-E-Dispo-model-v1 artifacts were not present in output_dir when the plus-sex sensitivity script ran."
    )
    comparison <- rbind(base_row, plus_row)
  }

  utils::write.csv(comparison, file.path(output_dir, OUTPUT_FILES[["comparison"]]), row.names = FALSE, na = "")
}
...
write_model_spec <- function(input_n, input_events, complete_case_n, complete_case_events, package_status) {
  ...
  lines <- c(
    ...
    '  "model_role": "prespecified_parallel_general_nontrauma_plus_sex_sensitivity",',
    '  "active_app_model": false,',
    '  "replaces_e_dispo_v4_0": false,',
    '  "replaces_general_e_dispo_model_v1": false,',
    ...
    '  },',
    '  "excluded_from_primary_formula": ["pain", "vomiting", "nausea", "hematemesis", "payer", "race_ethnicity", "region", "MSA", "imaging", "medication", "length_of_visit", "wait_time", "treatment", "diagnosis", "disposition_derived_fields"],',
    ...
    '    "sex is fairness-sensitive and is not endorsed for active use by apparent performance alone",',
...
write_blocker_outputs <- function(reason, input_n = 0L, input_events = 0L) {
  ...
  write_sensitivity_comparison(performance, calibration, optimism, reason)
  write_model_spec(input_n, input_events, 0L, 0L, NULL)
  writeLines(c(
    "# general-E-Dispo-model-v1-plus-sex",
    "",
    ...
    "",
    "This plus-sex sensitivity artifact does not replace active `e-dispo-v4.0` or base `general-E-Dispo-model-v1`.",
    ""
...
if (length(sex_levels) == 0) {
  write_blocker_outputs("The harmonized sex column is present but all values are missing in the admit-vs-home denominator.", input_n, input_events)
  quit(status = 2)
}
if (length(sex_levels) < 2) {
  write_blocker_outputs(
    paste0("The harmonized sex column has only one non-missing level in the admit-vs-home denominator: ", paste(sex_levels, collapse = ", ")),
    input_n,
    input_events
  )
  quit(status = 2)
...
if (length(complete_sex_levels) < 2) {
  write_blocker_outputs(
    paste0("The harmonized sex column has fewer than two non-missing levels after complete-case restriction: ", paste(complete_sex_levels, collapse = ", ")),
    input_n,
    input_events
  )
  quit(status = 2)
...
coefficients <- data.frame(
  ...
  evidence_tier = "survey_weighted_parallel_sensitivity_model",
  limitation_note = "Prespecified plus-sex sensitivity artifact only; not active e-dispo-v4.0, not a replacement for base general-E-Dispo-model-v1, not external validation, and not clinical-use evidence.",
  stringsAsFactors = FALSE
)
...
metrics <- metric_summary(complete_case, predicted, complete_case$weight, status_prefix = "survey_weighted_parallel_sensitivity_model")
performance <- data.frame(
  model_id = MODEL_ID, dataset = DATASET, target = TARGET, source_scope = SOURCE_SCOPE,
  ...
  status = "survey_weighted_parallel_sensitivity_model",
  limitation = "Prespecified plus-sex sensitivity artifact only; no replacement of e-dispo-v4.0 or base general-E-Dispo-model-v1, no clinical-use claim, and no external-validation claim.",
  stringsAsFactors = FALSE
)
...
sex_gap_max_abs <- function(frame) {
  if (is.null(frame) || nrow(frame) == 0 || !all(c("subgroup", "status", "calibration_gap") %in% names(frame))) return(NA_real_)
  rows <- frame[frame$subgroup == "sex" & frame$status == "ok", , drop = FALSE]
  if (nrow(rows) == 0) return(NA_real_)
  max(abs(as.numeric(rows$calibration_gap)), na.rm = TRUE)
}

comparison_lines <- c(
  "## Comparison With Base General Model",
  ""
)
base_performance_path <- file.path(output_dir, "general_e_dispo_model_v1_performance.csv")
base_optimism_path <- file.path(output_dir, "general_e_dispo_model_v1_optimism_corrected_performance.csv")
base_subgroup_path <- file.path(output_dir, "general_e_dispo_model_v1_subgroup_performance.csv")
if (file.exists(base_performance_path)) {
  base_performance <- read.csv(base_performance_path, stringsAsFactors = FALSE)
  comparison_lines <- c(
    comparison_lines,
    paste0("- Base AUROC: ", round(as.numeric(base_performance$auroc[[1]]), 6)),
    paste0("- Plus-sex AUROC: ", round(performance$auroc[[1]], 6)),
    paste0("- AUROC delta, plus-sex minus base: ", round(performance$auroc[[1]] - as.numeric(base_performance$auroc[[1]]), 6)),
    paste0("- Base Brier score: ", round(as.numeric(base_performance$brier_score[[1]]), 6)),
    paste0("- Plus-sex Brier score: ", round(performance$brier_score[[1]], 6)),
    paste0("- Brier delta, plus-sex minus base: ", round(performance$brier_score[[1]] - as.numeric(base_performance$brier_score[[1]]), 6))
  )
} else {
  comparison_lines <- c(comparison_lines, "- Base performance artifact was missing; comparison CSV includes a blocker row.")
}
if (file.exists(base_optimism_path)) {
  base_optimism <- read.csv(base_optimism_path, stringsAsFactors = FALSE)
  base_opt_auroc <- optimism_metric(base_optimism, "auroc")
  base_opt_brier <- optimism_metric(base_optimism, "brier_score")
  plus_opt_auroc <- optimism_metric(optimism, "auroc")
  plus_opt_brier <- optimism_metric(optimism, "brier_score")
  comparison_lines <- c(
    comparison_lines,
    paste0("- Optimism-corrected AUROC delta: ", round(plus_opt_auroc - base_opt_auroc, 6)),
    paste0("- Optimism-corrected Brier delta: ", round(plus_opt_brier - base_opt_brier, 6))
  )
}
if (file.exists(base_subgroup_path)) {
  base_subgroup <- read.csv(base_subgroup_path, stringsAsFactors = FALSE)
  base_sex_gap <- sex_gap_max_abs(base_subgroup)
  plus_sex_gap <- sex_gap_max_abs(subgroup_performance)
  if (is.finite(base_sex_gap) && is.finite(plus_sex_gap)) {
    comparison_lines <- c(
      comparison_lines,
      paste0("- Maximum absolute sex subgroup calibration gap, base: ", round(base_sex_gap, 6)),
      paste0("- Maximum absolute sex subgroup calibration gap, plus-sex: ", round(plus_sex_gap, 6)),
      paste0("- Sex subgroup calibration-gap delta, plus-sex minus base: ", round(plus_sex_gap - base_sex_gap, 6))
    )
  }
}
comparison_lines <- c(
  comparison_lines,
  "",
  "Negative Brier and sex-gap deltas are improvements. Positive AUROC deltas are improvements. These are internal NHAMCS sensitivity comparisons only."
)
...
report <- c(
  ...
  "Status: `survey_weighted_parallel_sensitivity_model`.",
  "",
  "This is a prespecified plus-sex sensitivity artifact for the parallel all-sex/all-age NHAMCS non-trauma educational/statistical model. It is not the active GitHub Pages model, not a replacement for `e-dispo-v4.0` or base `general-E-Dispo-model-v1`, not clinical decision support, not external validation, and not transportability evidence.",
  "",
  "## Method",
  "",
  ...
  "- Reference levels: `sex = 1`; `acuity_code = urgent`; `arrival_transfer_context = no_not_transferred_from_hospital_or_urgent_care`.",
  "- Excluded from fitted formula: pain, vomiting, nausea, hematemesis, payer, race/ethnicity, region, MSA, imaging, medication, length-of-visit, wait-time, treatment, diagnosis, and disposition-derived variables.",
  "- Sex is presentation-available but fairness-sensitive; this pass tests sensitivity and does not endorse active use by apparent performance alone.",
  "",
  "## Counts",
  "",
  ...
  "## Boundary",
  "",
  "This sensitivity model remains a reviewer/development artifact. It does not alter app behavior, does not broaden the intended-use boundary of `e-dispo-v4.0`, does not replace base `general-E-Dispo-model-v1`, and does not support patient-level clinical use.",
  ""
)
...
write_sensitivity_comparison(
  ...
  calibration,
  optimism,
  "Prespecified plus-sex sensitivity artifact only; compare against base general-E-Dispo-model-v1 before considering any model change."
)
...
cat("Wrote general-E-Dispo-model-v1-plus-sex outputs under ", output_dir, "\n", sep = "")
